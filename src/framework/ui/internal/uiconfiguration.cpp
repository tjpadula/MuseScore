...
static QColor colorFromHex(const QString& hex)
{
    ...
    bool aOK;
    int anAlpha = str.mid(1, 2).toInt(&aOK, 16);
    int aRed = str.mid(3, 2).toInt(&aOK, 16);
    int aGreen = str.mid(5, 2).toInt(&aOK, 16);
    int aBlue = str.mid(7, 2).toInt(&aOK, 16);

    return QColor::fromRgb(aRed, aGreen, aBlue, anAlpha);
